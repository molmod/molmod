...
class PSFFile(object):
    ...
    def clear(self):
        ...
        self.symbols = [] # the symbols in the second column, used to associate ff parameters
    ...
    def add_molecule(self, molecule, symbols=None, split=True):
        molecular_graph = generate_molecular_graph(molecule)
        self.add_molecular_graph(molecular_graph, symbols, split)

    def add_molecular_graph(self, molecular_graph, symbols=None, split=True):
        ...
        if symbols is None:
            symbols = [periodic[number].symbol for number in molecular_graph.numbers]
