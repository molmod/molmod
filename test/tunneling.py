...
from molmod.tunneling import eckart, wigner, print_batch
from molmod.units import kjmol, invcm
...
class TunnelingTestCase(unittest.TestCase):
    def test_leo(self):
        ...
        m1 = periodic[1].mass
        m2 = numpy.array([
            periodic[1].mass + periodic[1].mass,
            periodic[1].mass + periodic[6].mass+periodic[7].mass,
            periodic[1].mass + periodic[9].mass,
            periodic[1].mass + periodic[3].mass,
            periodic[9].mass + periodic[3].mass,
        ], float) + 2*periodic[6].mass + 4*periodic[1].mass
        mus = m1*m2/(m1+m2)
        
        for Ef, Er, fs, cs, mu in zip(forward, reverse, frequencies, eckart_corrections, mus):
            for f, c in zip(fs, cs):
                our = eckart(T, nu=f, Ef=Ef, Er=Er, mu=mu)[0]
